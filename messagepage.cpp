...
static QString normalizedImagePath(const QString &raw)
{
    ...
    const QFileInfo cwd(QDir::current(), p);
    if (cwd.exists())
        return cwd.absoluteFilePath();
    return p;
}

static QPixmap loadPixmapRobust(const QString &rawPath)
{
    const QString path = normalizedImagePath(rawPath);
    if (path.isEmpty())
        return QPixmap();
...
static QPixmap makeCirclePixmap(const QString &path, int size) {
    QPixmap px = loadPixmapRobust(path);
...
//  LOAD CURRENT USER
// ═══════════════════════════════════════════════════════
void MessagePage::loadCurrentUser(int userID, const QString &name, const QString &email) {
    m_currentUserName = name;

    if (sessionEmailLabel) {
        if (email.trimmed().isEmpty()) {
            sessionEmailLabel->hide();
        } else {
            sessionEmailLabel->setText(email.trimmed());
            sessionEmailLabel->show();
        }
    }
    ...
    if (m_currentUserID != userID) {
        ...
        messageInput->clear();
        if (renameGroupBtn)
            renameGroupBtn->setVisible(false);
        if (addMembersBtn)
            addMembersBtn->setVisible(false);
        if (manageGroupBtn)
            manageGroupBtn->setVisible(false);
    }
}

void MessagePage::resetForNewSession() {
    m_currentUserID  = -1;
    m_openChatUserID = -1;
    m_chatKind       = ChatKind::None;
    m_activeGroupId  = -1;
...
void MessagePage::syncListSelectionWithOpenChat() {
    for (auto *item : chatItems)
        item->setActive(item->getUserID() == m_openChatUserID);
}
...
void MessagePage::openGroupConversation(int groupID,
                                       const QString &groupName,
                                       const QString &iconPath,
                                       int memberCount,
                                       bool creatorCanManageMembers)
{
    m_openChatUserID = -groupID;
    m_chatKind       = ChatKind::Group;
    m_activeGroupId  = groupID;

    chatNameLabel->setText(groupName);
    const QString sub = QString::number(memberCount) + " members · Group chat";
    chatStatusLabel->setText(sub);
    chatStatusLabel->setObjectName("statusOffline");
    chatStatusLabel->style()->unpolish(chatStatusLabel);
    chatStatusLabel->style()->polish(chatStatusLabel);

    const QString path = iconPath.trimmed();
    if (!path.isEmpty()) {
        QPixmap circle = makeCirclePixmap(path, 38);
        if (!circle.isNull()) {
            chatAvatarLabel->setText("");
            ...
            chatAvatarLabel->setPixmap(QPixmap());
            chatAvatarLabel->setText(makeInitials(groupName));
        }
    } else {
        chatAvatarLabel->setPixmap(QPixmap());
        chatAvatarLabel->setText(makeInitials(groupName));
    ...
    if (renameGroupBtn)
        renameGroupBtn->setVisible(true);
    if (addMembersBtn)
        addMembersBtn->setVisible(true);
    if (manageGroupBtn)
        manageGroupBtn->setVisible(creatorCanManageMembers);
...
//  SLOTS
// ═══════════════════════════════════════════════════════
void MessagePage::onRenameGroupClicked() {
    if (m_chatKind != ChatKind::Group || m_activeGroupId <= 0) return;
    emit renameGroupClicked(m_activeGroupId);
}

void MessagePage::onAddMembersClicked() {
    if (m_chatKind != ChatKind::Group || m_activeGroupId <= 0) return;
    emit addMembersToGroupClicked(m_activeGroupId);
}

void MessagePage::onManageGroupClicked() {
    if (m_chatKind != ChatKind::Group || m_activeGroupId <= 0) return;
    emit manageGroupMembersClicked(m_activeGroupId);
}
...
void MessagePage::onSendClicked() {
    ...
    if (m_chatKind == ChatKind::Group && m_activeGroupId > 0) {
        QString time = QDateTime::currentDateTime().toString("hh:mm AP");
        addMessage(text, time, true);
        messageInput->clear();
        emit groupMessageSendRequested(m_activeGroupId, text);
        return;
    }

    if (m_chatKind == ChatKind::Dm && m_openChatUserID >= 0) {
        QString time = QDateTime::currentDateTime().toString("hh:mm AP");
        addMessage(text, time, true);
        messageInput->clear();
        emit dmMessageSendRequested(m_openChatUserID, text);
    }
}
...
void MessagePage::setupUI() {
    ...
    sessionEmailLabel = new QLabel();
    sessionEmailLabel->setObjectName("sessionEmailLabel");
    sessionEmailLabel->setWordWrap(true);
    sessionEmailLabel->hide();
    ...
    lhVl->addWidget(sessionEmailLabel);
    ...
    connect(createGroupBtn, &QPushButton::clicked,
            this, &MessagePage::createGroupClicked);
    lhVl->addWidget(createGroupBtn);
    ...
    renameGroupBtn = new QPushButton("Edit name");
    renameGroupBtn->setObjectName("renameGroupBtn");
    renameGroupBtn->setCursor(Qt::PointingHandCursor);
    renameGroupBtn->setFixedHeight(32);
    renameGroupBtn->hide();
    connect(renameGroupBtn, &QPushButton::clicked,
            this, &MessagePage::onRenameGroupClicked);
    ctHl->addWidget(renameGroupBtn);

    addMembersBtn = new QPushButton("Add members");
    addMembersBtn->setObjectName("addMembersBtn");
    addMembersBtn->setCursor(Qt::PointingHandCursor);
    addMembersBtn->setFixedHeight(32);
    addMembersBtn->hide();
    connect(addMembersBtn, &QPushButton::clicked,
            this, &MessagePage::onAddMembersClicked);
    ctHl->addWidget(addMembersBtn);

    manageGroupBtn = new QPushButton("Manage");
    manageGroupBtn->setObjectName("manageGroupBtn");
    manageGroupBtn->setCursor(Qt::PointingHandCursor);
    manageGroupBtn->setFixedHeight(32);
    manageGroupBtn->hide();
    connect(manageGroupBtn, &QPushButton::clicked,
            this, &MessagePage::onManageGroupClicked);
    ctHl->addWidget(manageGroupBtn);
...
void MessagePage::applyStyles() {
    setStyleSheet(R"(
        ...
        #sessionEmailLabel { font-size:11px; color:#9ca3af; }
        ...
        #createGroupBtn { background:rgba(124,58,237,0.2);
                         border:1px solid #2d1b69; border-radius:8px;
                         color:#c4b5fd; font-size:12px; font-weight:600; }
        #createGroupBtn:hover { background:rgba(124,58,237,0.35); }
        ...
        #renameGroupBtn { background:#12122a;
                         border:1px solid #2d2d5a; border-radius:8px;
                         color:#a78bfa; font-size:11px; font-weight:600;
                         padding:0 12px; }
        #renameGroupBtn:hover { border-color:#7c3aed; color:#ddd; }

        #addMembersBtn { background:#12122a;
                         border:1px solid #2d2d5a; border-radius:8px;
                         color:#a78bfa; font-size:11px; font-weight:600;
                         padding:0 12px; }
        #addMembersBtn:hover { border-color:#7c3aed; color:#ddd; }

        #manageGroupBtn { background:rgba(52,211,153,0.15);
                         border:1px solid #065f46; border-radius:8px;
                         color:#34d399; font-size:11px; font-weight:600;
                         padding:0 10px; }
        #manageGroupBtn:hover { background:rgba(52,211,153,0.28); }
