                               ...
                               bool isOwn)
{
    ...
    if (headerEmailLabel) {
        headerEmailLabel->setText(email.isEmpty() ? QString()
                                                  : QStringLiteral("✉️  %1").arg(email));
        headerEmailLabel->setVisible(!email.isEmpty());
    }
    ...
    if (aboutEmailLabel) {
        aboutEmailLabel->setText(QString("✉️  %1").arg(email.isEmpty() ? "Not set" : email));
    }
    ...
    if (reportUserBtn) {
        const bool canReport = isOwn == false && !isAdminProfile;
        reportUserBtn->setVisible(canReport);
    }
}
...
void ProfilePage::setupUI() {
    ...
    reportUserBtn = new QPushButton("⚠  Report");
    reportUserBtn->setObjectName("reportUserBtn");
    reportUserBtn->setFixedHeight(34);
    reportUserBtn->setCursor(Qt::PointingHandCursor);
    reportUserBtn->hide();
    avatarRow->addWidget(reportUserBtn);
    avatarRow->addSpacing(8);
    ...
    headerEmailLabel = new QLabel("✉️");
    headerEmailLabel->setObjectName("profileHeaderEmail");
    headerEmailLabel->setWordWrap(true);
    headerVl->addWidget(headerEmailLabel);
    ...
    aboutEmailLabel = new QLabel("✉️  Not set");
    aboutEmailLabel->setObjectName("aboutRow");
    aboutVl->addWidget(aboutEmailLabel);
    ...
    connect(reportUserBtn, &QPushButton::clicked, this, [this]() {
        if (m_profileUserID >= 0) emit reportUserClicked(m_profileUserID);
    });
...
void ProfilePage::applyStyles() {
    setStyleSheet(R"(
        ...
        #profileHeaderEmail { font-size:12px; color:#9ca3af; }
        ...
        #reportUserBtn { background:rgba(251,191,36,0.12);
                         color:#fbbf24;
                         border:1px solid rgba(251,191,36,0.35);
                         border-radius:8px;
                         font-size:12px; font-weight:600; padding:0 12px; }
        #reportUserBtn:hover { background:rgba(251,191,36,0.22); }
