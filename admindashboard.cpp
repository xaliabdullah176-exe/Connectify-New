...
//  ADD USER ROW
// ═══════════════════════════════════════════════════════
void AdminDashboard::addUserRow(int userID, QString name, QString email, QString avatarPath, QString role,
                                int postCount, bool isBanned)
{
    // Insert before stretch at end
    userTableLayout->insertWidget(
        userTableLayout->count() - 1,
        makeUserRow(userID, name, email, avatarPath, role, postCount, isBanned)
        );
...
void AdminDashboard::clearModerationLists() {
    if (!reportRowsLayout || !appealRowsLayout) return;
    while (reportRowsLayout->count() > 1) {
        QLayoutItem *it = reportRowsLayout->takeAt(0);
        delete it->widget();
        delete it;
    }
    while (appealRowsLayout->count() > 1) {
        QLayoutItem *it = appealRowsLayout->takeAt(0);
        delete it->widget();
        delete it;
    }
}

void AdminDashboard::addReportRow(int reportedUserID, int reportCount, const QString &summaryText) {
    if (!reportRowsLayout) return;
    reportRowsLayout->insertWidget(
        reportRowsLayout->count() - 1,
        makeReportCard(reportedUserID, reportCount, summaryText));
}

void AdminDashboard::addAppealRow(int userID, const QString &userName, const QString &message) {
    if (!appealRowsLayout) return;
    appealRowsLayout->insertWidget(
        appealRowsLayout->count() - 1,
        makeAppealCard(userID, userName, message));
}
...
void AdminDashboard::setActiveSidebarBtn(QPushButton *active) {
    for (auto btn : {navDashBtn, navUsersBtn, navPostsBtn, navModerationBtn, navNewsBtn}) {
...
//  MAKE USER ROW
// ═══════════════════════════════════════════════════════
QFrame* AdminDashboard::makeUserRow(int userID, QString name, QString email, QString avatarPath,
                                    ...
                                    bool isBanned)
{
    ...
    QLabel *emailLbl = new QLabel(email.trimmed().isEmpty() ? "—" : email.trimmed());
    emailLbl->setObjectName("tableUserEmail");
    emailLbl->setWordWrap(true);
    nameCol->addWidget(uname);
    nameCol->addWidget(uid);
    nameCol->addWidget(emailLbl);

    uc->addWidget(av);
    uc->addLayout(nameCol);
    userCell->setFixedWidth(220);
    ...
    QPushButton *groupsBtn = new QPushButton("Groups");
    groupsBtn->setObjectName("btnView");
    groupsBtn->setFixedHeight(26);
    groupsBtn->setCursor(Qt::PointingHandCursor);
    actions->addWidget(groupsBtn);
    connect(groupsBtn, &QPushButton::clicked, this,
            [this, userID]() { emit userGroupsClicked(userID); });
...
QFrame* AdminDashboard::makeReportCard(int reportedUserID, int reportCount,
                                       const QString &summary)
{
    ...
    QLabel *head = new QLabel(QStringLiteral("⚠️  %1 report(s) — user ID %2")
                                  .arg(reportCount)
                                  .arg(reportedUserID));
    head->setObjectName("modUser");
    vl->addWidget(head);

    QLabel *txt = new QLabel(summary);
    txt->setObjectName("modContent");
    txt->setWordWrap(true);
    vl->addWidget(txt);

    QHBoxLayout *hl = new QHBoxLayout();
    hl->setSpacing(8);
    QPushButton *banBtn = new QPushButton(QStringLiteral("Ban user"));
    banBtn->setObjectName("btnBan");
    banBtn->setCursor(Qt::PointingHandCursor);
    banBtn->setFixedHeight(28);
    QPushButton *ignBtn = new QPushButton(QStringLiteral("Ignore reports"));
    ignBtn->setObjectName("btnView");
    ignBtn->setCursor(Qt::PointingHandCursor);
    ignBtn->setFixedHeight(28);
    connect(banBtn, &QPushButton::clicked, this,
            [this, reportedUserID]() { emit moderationBanFromReport(reportedUserID); });
    connect(ignBtn, &QPushButton::clicked, this,
            [this, reportedUserID]() { emit moderationIgnoreReports(reportedUserID); });
    hl->addWidget(banBtn);
    hl->addWidget(ignBtn);
    hl->addStretch();
    vl->addLayout(hl);
...
QFrame* AdminDashboard::makeAppealCard(int userID, const QString &userName,
                                       const QString &message)
{
    ...
    QLabel *head = new QLabel(QStringLiteral("📝  Ban appeal — @%1 (ID %2)")
                                  .arg(userName)
                                  .arg(userID));
    head->setObjectName("modUser");
    vl->addWidget(head);
    ...
    QHBoxLayout *hl = new QHBoxLayout();
    QPushButton *unbanBtn = new QPushButton(QStringLiteral("Unban"));
    unbanBtn->setObjectName("btnView");
    unbanBtn->setCursor(Qt::PointingHandCursor);
    unbanBtn->setFixedHeight(28);
    QPushButton *keepBtn = new QPushButton(QStringLiteral("Keep banned"));
    keepBtn->setObjectName("btnBan");
    keepBtn->setCursor(Qt::PointingHandCursor);
    keepBtn->setFixedHeight(28);
    connect(unbanBtn, &QPushButton::clicked, this,
            [this, userID]() { emit moderationUnbanFromAppeal(userID); });
    connect(keepBtn, &QPushButton::clicked, this,
            [this, userID]() { emit moderationKeepBannedFromAppeal(userID); });
    hl->addWidget(unbanBtn);
    hl->addWidget(keepBtn);
    hl->addStretch();
    vl->addLayout(hl);
    return card;
}
...
void AdminDashboard::setupUI() {
    ...
    makeNavBtn(navModerationBtn, "⚠️", "Reports");
    ...
    // ── PAGE 3: Reports & ban appeals ─────────────────────
    moderationPage = new QWidget();
    QScrollArea *modScroll = new QScrollArea();
    modScroll->setWidgetResizable(true);
    modScroll->setFrameShape(QFrame::NoFrame);

    QWidget *modInner = new QWidget();
    QVBoxLayout *modOuterVl = new QVBoxLayout(modInner);
    modOuterVl->setContentsMargins(20, 16, 20, 20);
    modOuterVl->setSpacing(16);

    QLabel *repSec = new QLabel("User reports");
    repSec->setObjectName("sectionTitle");
    modOuterVl->addWidget(repSec);
    QLabel *repHint = new QLabel(
        "Each row shows how many users reported the same account. Ban applies a ban and clears "
        "reports; Ignore dismisses reports without banning. The reported user is not notified.");
    repHint->setObjectName("quickDesc");
    repHint->setWordWrap(true);
    modOuterVl->addWidget(repHint);

    QWidget *repWrap = new QWidget();
    reportRowsLayout = new QVBoxLayout(repWrap);
    reportRowsLayout->setSpacing(10);
    reportRowsLayout->setContentsMargins(0, 0, 0, 0);
    reportRowsLayout->addStretch();
    modOuterVl->addWidget(repWrap);

    QLabel *apSec = new QLabel("Ban appeals");
    apSec->setObjectName("sectionTitle");
    modOuterVl->addWidget(apSec);

    QWidget *apWrap = new QWidget();
    appealRowsLayout = new QVBoxLayout(apWrap);
    appealRowsLayout->setSpacing(10);
    appealRowsLayout->setContentsMargins(0, 0, 0, 0);
    appealRowsLayout->addStretch();
    modOuterVl->addWidget(apWrap);

    modOuterVl->addStretch();
    modScroll->setWidget(modInner);
    QVBoxLayout *modPageVl = new QVBoxLayout(moderationPage);
    modPageVl->setContentsMargins(0, 0, 0, 0);
    modPageVl->addWidget(modScroll);
    contentStack->addWidget(moderationPage);   // index 3
    ...
    connect(navModerationBtn, &QPushButton::clicked, this, [this]() {
        contentStack->setCurrentIndex(3);
        pageTitleLabel->setText("Reports & appeals");
        pageSubLabel->setText("User reports and banned-account appeals");
        setActiveSidebarBtn(navModerationBtn);
    });
...
void AdminDashboard::applyStyles() {
    setStyleSheet(R"(
        ...
        #tableUserEmail  { font-size:10px; color:#9ca3af; }
