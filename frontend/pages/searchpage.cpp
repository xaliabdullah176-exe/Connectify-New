    ...
    : QFrame(parent), m_userID(user.userID)
{
    ...
    if (!user.email.trimmed().isEmpty()) {
        QLabel *emailLbl = new QLabel(user.email.trimmed());
        emailLbl->setObjectName("resultEmail");
        QFontMetrics fem(emailLbl->font());
        emailLbl->setText(fem.elidedText(user.email.trimmed(), Qt::ElideRight, 340));
        infoVl->addWidget(emailLbl);
    }
    ...
    QString friendBtnText = "+ Add Friend";
    QString friendBtnObj = "btnAddFriend";
    bool friendBtnDisabled = false;
    if (user.isFriend) {
        friendBtnText = "✓  Friends";
        friendBtnObj = "btnAlreadyFriend";
        friendBtnDisabled = true;
    } else if (user.requestSent) {
        friendBtnText = "⏳  Request sent";
        friendBtnObj = "btnRequestSent";
        friendBtnDisabled = true;
    }
    QPushButton *friendBtn = new QPushButton(friendBtnText);
    friendBtn->setObjectName(friendBtnObj);
    friendBtn->setFixedHeight(34);
    friendBtn->setFixedWidth(124);
    friendBtn->setCursor(friendBtnDisabled ? Qt::ArrowCursor : Qt::PointingHandCursor);
    friendBtn->setEnabled(!friendBtnDisabled);

    connect(viewBtn, &QPushButton::clicked, this,
        [this]() { emit viewProfileClicked(m_userID); });
    if (!friendBtnDisabled) {
...
void SearchPage::loadCurrentUser(int userID, const QString &name, const QString &avatarPath,
    ...
    const QString chip = email.trimmed().isEmpty() ? name : (name + " · " + email.trimmed());
    currentUserNameLabel->setText(chip);
...
void SearchPage::addUser(int userID, const QString &name,
                          const QString &username, const QString &email, const QString &bio, const QString &avatarPath,
                          bool isFriend, bool requestSent)
{
    m_allUsers.append({userID, name, username, email, bio, avatarPath, isFriend, requestSent});
    // Show all by default
...
void SearchPage::setupUI() {
    ...
    searchInput->setPlaceholderText("Search by name, @username, or email...");
...
void SearchPage::applyStyles() {
    setStyleSheet(R"(
        ...
        #resultEmail    { font-size:11px; color:#9ca3af; }
        ...
        #btnRequestSent { background:rgba(251,191,36,0.12);
                          color:#fbbf24;
